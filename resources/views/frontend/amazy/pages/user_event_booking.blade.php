                            ...
                            <div class="dashboard_white_box_body">
                                <div class="table-responsive mb_10">
                                    <table class="table amazy_table3 style2 mb-0">
                                        ...
                                        <tbody>
                                            @foreach ($bookingevents as $key => $booking)
                                                <tr>
                                                    <td>{{$booking->id}}</td>
                                                    <td>{{$booking->user->email}}</td>
                                                    <td>{{$booking->purchase_date}}</td>
                                                    <td>{{single_price($booking->event->price * $booking->no_of_ticket)}}</td>
                                                    <td><a href="{{ route('frontend.user.book.event.show', $booking->id) }}" class="amaz_primary_btn style2 text-nowrap">{{__('Event Details')}}</a></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
