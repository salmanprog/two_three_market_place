                            ...
                            @foreach($topnavbar_left_menu->elements->where('has_parent',null) as $element)
                                ...
                                @elseif($element->type == 'page' && $element->page->status == 1)
                                    ...
                                    @if(!isModuleActive('MultiVendor') && $element->page->slug == 'merchant' || !isModuleActive('MultiVendor') && $element->page->module == 'MultiVendor')
                                        @continue
                                    @endif
                                        <li><a  href="{{ url(@$element->page->slug) }}">{{ ucfirst(textLimit($element->title, 25)) }}</a></li>
                                @elseif($element->type == 'category')

                                <li>
                                    <a href="{{route('frontend.category-product',['slug' => $element->category->slug, 'item' =>'category'])}}">{{ $element->title }}</a>
                                </li>

                                @elseif($element->type == 'brand')
                                <li>
                                    <a
                                        href="{{route('frontend.category-product',['slug' => $element->brand->slug, 'item' =>'brand'])}}">{{ $element->title }}</a>
                                </li>
                                @elseif($element->type == 'tag')
                                <li>
                                    <a
                                        href="{{route('frontend.category-product',['slug' => $element->tag->name, 'item' =>'tag'])}}">{{ $element->title }}</a>
                                </li>
                                @elseif($element->type == 'product' && @$element->product)
                                <li>
                                    <a href="{{singleProductURL(@$element->product->seller->slug, @$element->product->slug)}}">{{ $element->title }}</a>
                                </li>
                                @endif
                            @endforeach
                        @endif
                                            ...
                                            <div class="user_account_details">
                                                ...
                                                @else
                                                    ...
                                                    __('customer_panel.my_account') }}</a>
                                                        ...
                                                        @elseif($element->type == 'category')
                                                            <a
                                                                href="{{route('frontend.category-product',['slug' => $element->category->slug, 'item' =>'category'])}}">{{ $element->title }}</a>
                                                        @elseif($element->type == 'brand')
                                                            <a
                                                                href="{{route('frontend.category-product',['slug' => $element->brand->slug, 'item' =>'brand'])}}">{{ $element->title }}</a>
                                                        @elseif($element->type == 'tag')
