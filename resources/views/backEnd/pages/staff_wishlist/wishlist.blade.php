...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function fetch_filter_data(page){
            ...
            if (sort_by != null && paginate != null) {
                var url = "{{route('event.my-wishlist.paginate-data')}}"+'?sort_by='+sort_by+'&paginate='+paginate+'&page='+page;
            }else if (sort_by == null && paginate != null) {
                var url = "{{route('event.my-wishlist.paginate-data')}}"+'?paginate='+paginate+'&page='+page;
            }else {
                var url = "{{route('event.my-wishlist.paginate-data')}}"+'?page='+page;
            }
            if(page != 'undefined'){
                $.ajax({
                    url:url,
    ...
    function getFilterUpdateByIndex(){
        ...
        var sort_by = $('#product_short_list').val();
        $("#pre-loader").removeClass('d-none');
        $.get("{{ route('event.my-wishlist.paginate-data') }}", {sort_by:sort_by, paginate:paginate}, function(data){
            $('#productShow').html(data);
            $('#product_short_list').niceSelect();
            $('#paginate_by').niceSelect();
            $("#pre-loader").addClass('d-none');
