...
class ArtGalleryRegisterController extends Controller
{
    ...
    protected function redirectTo()
    {
        ...
        if(session()->has('pricing_id')){
            return  redirect('/art-gallery/art-gallery-subscription-payment-select/'.encrypt(session()->get('seller_pricing')));
        }
        return redirect('/art-gallery/dashboard');
    }
    ...
    public function showRegisterSubscription(Request $request, $id)
    {
        ...
        if (app('business_settings')->where('category_type', 'vendor_configuration')->where('type', 'Multi-Vendor System Activate')->first()->status) {
            if (auth()->check() && auth()->user()->role->type == 'customer') {
                ...
                if ($commission->id == 3) {
                    $data['pricing_plans'] = Pricing::with('activeFeatures')->where('best_for', 'Art Gallery')->where('status', 1)->get();
                ...
                if ($commission->id == 3) {
                    $data['pricing_plans'] = Pricing::with('activeFeatures')->where('best_for', 'Art Gallery')->where('status', 1)->get();
                    $data['content'] = MerchantContent::firstOrFail();
                    return view(theme('pages.art-gallery_create_by_subscription'), $data);
                } else {
                    session()->forget('pricing_id');
                }
    ...
    public function showRegisterForm(Request $request)
    {
        ...
        if (app('business_settings')->where('category_type', 'vendor_configuration')->where('type', 'Multi-Vendor System Activate')->first()->status) {
            if (auth()->check() && auth()->user()->role->type == 'customer') {
                ...
                $data['pricing_plans'] = Pricing::where('best_for', 'Art Gallery')->where('status', 1)->get(['name', 'id']);
                $registerRepo = new MerchantRepository();
                $registerRepo->customerToSellerConvert([
                    'commission_id' => session()->get('commission_id'),
                    'commission_rate' => session()->get('commission_rate'),
                ...
                $data['pricing_plans'] = Pricing::where('best_for', 'Art Gallery')->where('status', 1)->get(['name', 'id']);
                return view(theme('pages.art-gallery_create_step_one'), $data);
            } else {
                return abort(404);
            }
        } else {
