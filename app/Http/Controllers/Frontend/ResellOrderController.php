...
class ResellOrderController extends Controller
{
    ...
    public function __construct(OrderService $orderService, SellerProductService $productService, OrderManageService $ordermanageService)
    {
    ...
    public function my_resell_purchase_order_index(Request $request)
    {
        // if ($request->has('rn')) {
        ...
        $data['orders'] = $this->ordermanageService->myConfirmedReSalesList(auth()->user()->id);
        
        return view(theme('pages.profile.resellorder'), $data);
    }
