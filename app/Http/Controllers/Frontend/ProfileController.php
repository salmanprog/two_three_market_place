    ...
    public function dashboard()
    {
        try{
            if (auth()->user()->role->type == "superadmin" || auth()->user()->role->type == "admin" || auth()->user()->role->type == "staff") {
                ...
                $data['totalOrganiser'] = User::where('role_id',3)->get()->count();
                $data['totalvisitors'] = VisitorHistory::VisitorCount('today');
                $data['total_sale'] = Order::TotalSaleCount('year');
                ...
                $data['total_order'] = Order::OrderInfo('year', 'all');
                $data['total_pending_order'] = Order::OrderInfo('year', 0);
                $data['total_completed_order'] = Order::OrderInfo('year', 1);
                $data['total_event'] = Event::AdminTotalEvents();
                $data['total_active_event'] = Event::AdminTotalActiveEvents();
                $data['total_expire_event'] = Event::AdminTotalExpireEvents();
                $data['total_event_bookings'] = EventBooking::AdminTotalBookings();
                $data['income'] = Transaction::GetIncome('year');
                $data['expense'] = Transaction::GetExpense('year');
